...
export default function UploadResumePage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // 1. POST to /api/resume to create a new resume
      ...
      alert('Failed to upload/create resume. Please try again.');
      setPhase('upload');
    }
  };
  ...
  return (
    <div className="flex min-h-[calc(100vh-56px)] items-center justify-center p-4">
      <div className="w-full max-w-xl rounded-[12px] border-2 border-dashed border-border bg-white p-8 text-center shadow-card transition-colors hover:border-primary-DEFAULT hover:bg-blue-50/40">
        ...
        <p className="mt-2 text-sm text-text-muted">Select a PDF or DOCX file to extract info & start editing.</p>
        ...
        <Button className="mt-6" onClick={handleBrowseClick}>Browse File</Button>
      </div>
    </div>
  );
}
